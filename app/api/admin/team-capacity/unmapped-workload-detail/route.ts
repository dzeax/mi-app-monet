...
const buildUserMaps = (users: AppUserRow[]) => {
  ...
  users.forEach((user) => {
    ...
    if (emailKey) {
      emailToUser.set(emailKey, user.user_id);
    }
  ...
  return { emailToUser, nameToUser };
};

const buildPersonToUser = (
  people: PeopleRow[],
...
const resolveUserId = (
  ...
  owner: string | null | undefined,
  {
    ...
    personToUser: Map<string, string>;
    aliasToPerson: Map<string, string>;
    emailToUser: Map<string, string>;
    nameToUser: Map<string, string>;
  },
) => {
...
export async function GET(req: Request) {
  ...
  const activeMaps = buildUserMaps(activeUsers);
  const inactiveMaps = buildUserMaps(inactiveUsers);
  const personToActiveUser = buildPersonToUser(people, activeMaps.emailToUser, activeMaps.nameToUser);
  const personToInactiveUser = buildPersonToUser(people, inactiveMaps.emailToUser, inactiveMaps.nameToUser);
  ...
  try {
    ...
    const resolveOwnerIdentity = (personId?: string | null, owner?: string | null) => {
      let resolvedPersonId = personId ?? null;
      if (!resolvedPersonId && owner) {
        const aliasKey = normalize(owner);
        const aliasPersonId = aliasKey ? aliasToPerson.get(aliasKey) : null;
        if (aliasPersonId) {
          resolvedPersonId = aliasPersonId;
        }
      }

      const person = resolvedPersonId ? peopleById.get(resolvedPersonId) : null;
      const ownerEmail =
        person?.email ??
        (owner && owner.includes('@') ? owner.trim() : null);
      const ownerName =
        person?.display_name ??
        (!ownerEmail && owner ? owner.trim() : null);

      const ownerKey = resolvedPersonId
      ...
      return {
        ...
        ownerName,
        ownerEmail,
        ownerPersonId: resolvedPersonId,
      };
    };
    ...
    const addDetail = (
      ...
      const key = `${detail.ownerKey}|${detail.source}|${detail.clientSlug ?? 'unassigned'}|${detail.label ?? 'general'}`;
      const current = detailMap.get(key) ?? { ...detail, hours: 0 };
      current.hours += detail.hours;
      current.inactiveMatch = current.inactiveMatch || inactiveMatch;
      detailMap.set(key, current);
    };

    contributions.forEach((row) => {
      const resolvedActiveUser = resolveUserId(row.person_id, row.owner, {
        personToUser: personToActiveUser,
        aliasToPerson,
        emailToUser: activeMaps.emailToUser,
        nameToUser: activeMaps.nameToUser,
      });
      const resolvedInactiveUser = resolveUserId(row.person_id, row.owner, {
        personToUser: personToInactiveUser,
        aliasToPerson,
        emailToUser: inactiveMaps.emailToUser,
        nameToUser: inactiveMaps.nameToUser,
      });
      const identity = resolveOwnerIdentity(row.person_id, row.owner);
    ...
    strategyEfforts.forEach((row) => {
      const resolvedActiveUser = resolveUserId(null, row.owner, {
        personToUser: personToActiveUser,
        aliasToPerson,
        emailToUser: activeMaps.emailToUser,
        nameToUser: activeMaps.nameToUser,
      });
      const resolvedInactiveUser = resolveUserId(null, row.owner, {
        personToUser: personToInactiveUser,
        aliasToPerson,
        emailToUser: inactiveMaps.emailToUser,
        nameToUser: inactiveMaps.nameToUser,
      ...
      const label = pickLabel(ticket?.category, ticket?.jira_ticket, ticket?.title) ?? 'Strategy';
      addDetail(resolvedActiveUser, Boolean(resolvedInactiveUser), {
        ...identity,
        inactiveMatch: Boolean(resolvedInactiveUser),
        clientSlug: row.client_slug ?? null,
    ...
    const items = Array.from(detailMap.values())
      .map((entry) => ({
        id: `${entry.ownerKey}-${entry.source}-${entry.clientSlug ?? 'unassigned'}-${entry.label ?? 'general'}`,
        ownerKey: entry.ownerKey,
        ownerName: entry.ownerName,
        ownerEmail: entry.ownerEmail,
        ownerPersonId: entry.ownerPersonId,
        inactiveMatch: entry.inactiveMatch,
    ...
    return NextResponse.json(
      { error: err instanceof Error ? err.message : 'Failed to load unmapped workload.' },
      { status: 500 },
    );
  }
}
