...
export default function CampaignTable() {
  ...
  const [sizes, setSizes] = useState({ filters: 0 });
  ...
  const stackedBottom = `calc(var(--content-sticky-top) + ${sizes.filters}px + ${bandGapPx}px)`;
    ...
    <div className="w-full px-2 md:px-3 lg:px-4">
      ...
      >
        ...
        >
          <CampaignFilters
            ...
            onOpenKpiDetails={() => setShowKpiDetails(true)}
          />
        </div>
        ...
        <div
          ...
          style={{
            position: 'sticky',
            top: 'var(--content-sticky-top)',
            zIndex: 20,      // por debajo del stack de filtros (60)
            height: 0,
            pointerEvents: 'none',
          }}
        >
          <div
            style={{
              ...
              left: '-0.5rem',
              right: '-0.5rem',
              height: `calc(${sizes.filters}px + ${bandGapPx}px)`,
      ...
      {showKpiDetails && (
        <MiniModal
          ...
          bodyClassName="pt-3 pb-6"
          footer={
            <button type="button" className="btn-primary px-4 py-2 text-sm" onClick={() => setShowKpiDetails(false)}>
              Close
            </button>
          }
        >
          <div className="flex flex-wrap items-center justify-between gap-3 text-[12px] md:text-sm">
            ...
            <span className="tabular-nums text-[color-mix(in_oklab,var(--color-text)_75%,black)]">
              Routing: <strong>{summaryKpis.routing}</strong> • QTY: <strong>{summaryKpis.qty}</strong>
            </span>
          </div>

          <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            <KpiTile label="Turnover" value={summaryKpis.turnover} title="Suma de turnover del dataset filtrado" />
            <KpiTile
              label="Margin (%)"
              value={summaryKpis.marginPct}
              tone={summaryKpis.marginTone}
              asBadge
              subValue={summaryKpis.marginValue}
              title="Margin% = Margin / Turnover"
            />
            <KpiTile label="V Sent" value={summaryKpis.vSent} title="Volumen de envíos en el periodo" />
            <KpiTile label="eCPM" value={summaryKpis.ecpm} rightHint="€/k" title="Weighted eCPM = Σ(ecpm·vSent) / Σ(vSent)" />
          </div>
        </MiniModal>
      )}
