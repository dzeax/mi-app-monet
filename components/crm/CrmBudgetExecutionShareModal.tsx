...
export default function CrmBudgetExecutionShareModal({
  open,
  onClose,
  clientSlug,
  yearOptions,
  ...
  useEffect(() => {
    if (!open) return;
    setSelectedYears((prev) => (prev.length ? prev : sortedYearOptions.slice(0, 1)));
  }, [open, sortedYearOptions]);

  const loadShares = async () => {
    setLoading(true);
    setError(null);
    try {
      const res = await fetch(`/api/crm/budget-execution-shares?client=${clientSlug}`);
  ...
  const createShare = async () => {
    ...
    setCopied(false);
    try {
      const res = await fetch("/api/crm/budget-execution-shares", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ client: clientSlug, years: selectedYears }),
      });
      const body = await res.json().catch(() => null);
      if (!res.ok) {
      ...
      if (token) {
        ...
        setLastLink(
          `${window.location.origin}/share/${clientSlug}/budget-execution/${token}?year=${year}`,
        );
      }
      await loadShares();
    } catch (err) {
  ...
  const revokeShare = async (id: string) => {
    setError(null);
    try {
      const res = await fetch(`/api/crm/budget-execution-shares/${id}`, { method: "DELETE" });
      const body = await res.json().catch(() => null);
      if (!res.ok) {
      ...
      <div className="space-y-6">
        <section className="rounded-2xl border border-[color:var(--color-border)] bg-white p-4">
          ...
          <p className="mt-2 text-sm text-[color:var(--color-text)]/70">
            Share links can be used by multiple people. Select the allowed years for this link.
          </p>
              ...
              <button
                ...
                className={`rounded-full border px-3 py-1 text-xs font-medium ${
                  selectedYears.includes(year)
                    ...
                    : "border-[color:var(--color-border)] text-[color:var(--color-text)]/70"
                }`}
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3">
            <button
              ...
              onClick={createShare}
              disabled={creating || selectedYears.length === 0}
            >
              ...
              >
                <div className="space-y-1">
                  <div className="font-semibold text-[color:var(--color-text)]">
                    Years: {share.allowed_years?.slice().sort((a, b) => b - a).join(", ") || "—"}
                  </div>
